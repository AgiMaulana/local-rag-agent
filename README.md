...
- **Smart ingestion**: Detects file changes to avoid re-processing unchanged documents
...
```bash
# Smart ingestion (only processes new/changed files)
python -m ingestion.ingest

# Full re-ingestion (ignores manifest, reprocesses everything)
python -m ingestion.ingest --all

# Ingest specific file(s)
python -m ingestion.ingest myfile.pdf another.docx
```
...
- Track processed files in a manifest to avoid redundant work
